...
class FashionDataset(Dataset):
    ...
    def initDB(self, count, start=0):
        ...
        all_images, classes_count, class_mapping = pickle.load(
            open("data.pk", "rb"))
        self.classes = {}
        # Add classes
        ...
        for k, item in enumerate(all_images[start:count+start]):
            self.add_image(source="Fashion", image_id=k,
                           path=item['filepath'], width=item['width'], height=item['height'], bboxes=item['bboxes'])
    ...
    def load_image(self, image_id):
        ...
        tempImg = np.array(Image.open(
            os.path.join(self.rootpath, info['path'])))
        return tempImg
...
if __name__ == '__main__':
    ...
    model = RFCN_Model(mode="training", config=config,
...
#     with open(file_path, 'rb') as fpkl, open('%s.json' % "", 'w') as fjson:
#         data = pickle.load(fpkl)
#         json.dump(data, fjson, ensure_ascii=False, sort_keys=True, indent=4)


# print("Processing %s ..." % "data.pk")
# convert_dict_to_json('data.pk')
