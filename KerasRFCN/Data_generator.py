...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    # FG
    fg_roi_count = int(200 * 0.33)
    if fg_ids.shape[0] > fg_roi_count:
        keep_fg_ids = np.random.choice(fg_ids, fg_roi_count, replace=False)
    else:
        keep_fg_ids = fg_ids
    # BG
    remaining = 200 - keep_fg_ids.shape[0]
...
def data_generator(dataset, shuffle=True, augment=True, random_rois=0,
    ...
    anchors = KerasRFCN.Utils.generate_pyramid_anchors((32, 64, 128, 256, 512),
                                                       [0.5, 1, 2],
                                                       np.array([[320, 320],
                                                                 [160, 160],
                                                                 [80,  80],
