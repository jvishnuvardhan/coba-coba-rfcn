...
class RFCN_Model:
    ...
    def __init__(self, mode, model_dir):
        ...
        self.IMAGE_SHAPE = np.array([1280, 1280, 3])
        self.set_log_dir()
    ...
    def build(self, mode):
        ...
        input_image = KL.Input(
            shape=self.IMAGE_SHAPE.tolist(), name="input_image")
        ...
        if mode == "training":
            ...
            image_scale = K.cast(
                K.stack([K.shape(input_image)[1], K.shape(input_image)[2], K.shape(input_image)[1], K.shape(input_image)[2]], axis=0), "float32")
            gt_boxes = KL.Lambda(
                lambda x: x / K.cast(K.stack([h, w, h, w], axis=0), "float32"))(input_gt_boxes)

        P2, P3, P4, P5, P6 = ResNet(
            input_image, architecture="resnet50").output_layers
        ...
        ### RPN ###
        rpn = self.build_rpn_model(1, len([0.5, 1, 2]), 256)
        ...
        print(np.array([[320, 320],
                        [160, 160],
                        [80,  80],
                        [40,  40],
                        [20,  20]]))
        self.anchors = KerasRFCN.Utils.generate_pyramid_anchors((32, 64, 128, 256, 512),
                                                                [0.5, 1, 2],
                                                                np.array([[320, 320],
                                                                          [160, 160],
                                                                          [80,  80],
        ...
        for feature_map_count, feature_map in enumerate(mrcnn_feature_maps):
            # [W * H * class_num] * k^2
            ScoreMap = KL.Conv2D(12 * scoreMapSize, kernel_size=(
        ...
        # and zero padded.
        proposal_count = 2000 if mode == "training"\
            else 1000
        rpn_rois = ProposalLayer(proposal_count=proposal_count,
                                 nms_threshold=0.7,
                                 name="ROI",
                                 anchors=self.anchors,
                                 IMAGE_SHAPE=self.IMAGE_SHAPE)([rpn_class, rpn_bbox])
        ...
        if mode == "training":
            ...
            rois, target_class_ids, target_bbox =\
                DetectionTargetLayer(self.IMAGE_SHAPE, name="proposal_targets")([
                    rpn_rois, input_gt_class_ids, gt_boxes])

            # size = [batch, num_rois, class_num]
            classify_vote = VotePooling(200, 12, 3, 7,
                                        1*1, self.IMAGE_SHAPE, name="classify_vote")([rois] + ScoreMaps_classify)
            classify_output = KL.TimeDistributed(KL.Activation(
                'softmax'), name="classify_output")(classify_vote)

            # 4 k^2 rather than 4k^2*C
            regr_vote = VotePooling(200, 4, 3, 7,
                                    1*1, self.IMAGE_SHAPE, name="regr_vote")([rois] + ScoreMaps_regr)
            ...
            rpn_bbox_loss = KL.Lambda(lambda x: KerasRFCN.Losses.rpn_bbox_loss_graph(*x), name="rpn_bbox_loss")(
            ...
            # size = [batch, num_rois, class_num]
            classify_vote = VotePooling(proposal_count, 12, 3, 7, 1*1,
                                        self.IMAGE_SHAPE, name="classify_vote")([rpn_rois] + ScoreMaps_classify)
            classify_output = KL.TimeDistributed(KL.Activation(
                'softmax'), name="classify_output")(classify_vote)

            # 4 k^2 rather than 4k^2*C
            regr_vote = VotePooling(proposal_count, 4, 3, 7, 1*1,
                                    self.IMAGE_SHAPE, name="regr_vote")([rpn_rois] + ScoreMaps_regr)
            regr_output = KL.TimeDistributed(KL.Activation(
                'linear'), name="regr_output")(regr_vote)
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            self.model.metrics.append(
                KL.Lambda(lambda x: K.mean(x, keepdims=True))(layer.output))
    ...
    def call(self, inputs):
        ...
        for channel_step in range(self.k*self.k):
            ...
            croped = KL.Lambda(lambda x: tf.image.crop_to_bounding_box(
                tf.gather(x, indices=channel_indices, axis=-
                          1), bin_y,
...
def refine_detections_graph(rois, probs, deltas, window):
    ...
    height, width = np.array([0.1, 0.1, 0.2, 0.2])
